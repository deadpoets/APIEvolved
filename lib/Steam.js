...
exports.UpdateSteamCMD = function (callback) {
    Config.Load(function (config) {
        fs.stat(config.SteamCMD + "\\steamcmd.exe", function(err, stats) {
            if(err) {
                var destPath = config.SteamCMD;
                ...
                fs.stat(destPath, function (err, stats) {
                    ...
                    Logger.log('info', "[Steam] Getting the latest SteamCMD to " + destPath);
                    request
                        .get('https://steamcdn-a.akamaihd.net/client/installer/steamcmd.zip')
                        .on('error', function (err) {
                            Logger.log('error', "[Steam] Error downloading: " + err);
                            callback(false);
                        })
                        .on('complete', function () {
                            Logger.log('info', "[Steam] Download complete.  Unpacking...");
                            new decompress({ mode: '777' })
                                ...
                                .run(function (err, files) {
                                    if (err) {
                                        Logger.log('error', "[Steam] Error unzipping: " + err);
                                        callback(false);
                                    }
                                    else {
                                        Logger.log('info', "[Steam] Unpack complete.  SteamCMD is up-to-date.");
                                        callback(true);
                                    }
                                });
                        })
...
exports.Update = function(callback) {
    Config.Load(function(config) {
        ...
        if(config.Server.AutoUpdate.Validate) {
            ...
            steam =  spawn(config.SteamCMD + "\\steamcmd.exe", ['+login', 'anonymous', '+force_install_dir', config.Server.Win64 + "\\..\\..\\..\\", "+app_update", "376030", "validate", "+quit"]);
        } else {
            // Will not validate Game server files
            steam =  spawn(config.SteamCMD  + "\\steamcmd.exe", ['+login', 'anonymous', '+force_install_dir', config.Server.Win64 + "\\..\\..\\..", "+app_update", "376030", "+quit"]);
        }
